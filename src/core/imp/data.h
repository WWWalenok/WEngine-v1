...
template<class type>
static void Read(size_t& offset, const data_t& var, type* ret)
{
    Read(offset, var, ret, sizeof(type));
}
...
template<class type>
static type Read(size_t& offset, const data_t& var)
{
    type ret;
    Read(offset, var, &ret, sizeof(type));
    return ret;
...
template<class type>
static void Write(size_t& offset, data_t& ret, const type* var)
{
    Write(offset, ret, var, sizeof(type));
}
...
struct Helper
{
    static size_t ID()
    {
        static const auto v = GID_V(typeid(T).name());
        return v;
    }
    static const char* NAME()
    {
        static const auto v = typeid(T).name();
        return v;
    }
};
template<typename T>
...
public:
    ...
    static bool RegisterConverter()
    {
        ...
        converters[Helper<From>::ID()][Helper<To>::ID()] = [](const void* from, void* to) {
            try
            {
    ...
    {
        auto& converters                                 = get_converters();
        converters[Helper<From>::ID()][Helper<To>::ID()] = [conv_func = func](const void* from, void* to) {
            try
            {
...
{
    inline static RawData* make(const char*& _data) { return RawValue<std::string>::make(_data); }
    inline static RawData* make(const char**& _data) { return RawValue<std::string>::make(*_data); }
};
...
struct _smart_pointer
{
    ...
    {
        return _smart_pointer(RawValue<T...>::make(value...));
    }
    ...
    _smart_pointer(std::nullptr_t) : counter(nullptr) {}

    _smart_pointer(const _smart_pointer<false>& other)
    {
        if (other.counter)
        {
            if constexpr (weak)
                counter = other.counter->winc();
            else
    ...
    _smart_pointer(const _smart_pointer<true>& other)
    {
        if (other.counter)
        {
            if constexpr (weak)
    ...
    _smart_pointer& operator=(const _smart_pointer<false>& other)
    {
        if ((void*)this != &other)
        {
            dec_impl();
    ...
    _smart_pointer& operator=(const _smart_pointer<true>& other)
    {
        if ((void*)this != &other)
        {
            dec_impl();
...
struct IBasePointer
{
    virtual void __to(IBasePointer* _) const = 0;
...
{
private:
    _smart_pointer<false> imp;

public:
    virtual void __to(IBasePointer* _) const { _->__from(imp); };

    virtual void __from(const _smart_pointer<true>& _) { imp = _; }
    virtual void __from(const _smart_pointer<false>& _) { imp = _; }

    BasePointer() {}

    BasePointer(const _smart_pointer<false>& _) { imp = _; }
    BasePointer(const _smart_pointer<true>& _) { imp = _; }

    BasePointer(const IBasePointer* _) { _->__to(this); }
    BasePointer& operator=(const IBasePointer* _)
    {
        _->__to(this);
        return *this;
    }
    ...
    template<typename T>
    void make_from(const T& value)
    {
        imp = _smart_pointer<false>(value);
    }

    template<typename T>
    void make_from_ptr(T* value)
    {
...
public:
    ...
    WeakBasePointer(const _smart_pointer<false>& _) { imp = _; }
    WeakBasePointer(const _smart_pointer<true>& _) { imp = _; }

    WeakBasePointer(const IBasePointer* _) { _->__to(this); }
    WeakBasePointer& operator=(const IBasePointer* _)
    {
        _->__to(this);
        return *this;
    }
...
struct WeakPointer : public WeakBasePointer
{
    ...
    WeakPointer(WeakPointer<T>& _) : WeakBasePointer(_)
    {}
    WeakPointer(WeakPointer<T>&& _) : WeakBasePointer(_)
    {}
    WeakPointer(const WeakPointer<T>& _) : WeakBasePointer(_)
    {}
    WeakPointer& operator=(const WeakPointer<T>& _)
    {
        _.__to(this);
        return *this;
    }
...
struct Data : public BasePointer
{
    ...
    template<typename T>
    Data(const T& value, void* owner = nullptr) : BasePointer(_smart_pointer<false>::make(value)), sender(owner)
    {}
    Data(const nullptr_t*& value, void* owner = nullptr) : BasePointer(_smart_pointer<false>()), sender(owner) {}
     ...
     */
    template<typename T, typename std::enable_if<!std::is_same<T, bool>::value && !std::is_base_of<IBasePointer, T>::value>::type* = nullptr>
    operator T&()
    {
        if (__data() == nullptr)
        ...
        if (t)
            return *t;
        auto s = std::string("Trying to get an incorrect type from data. Current: ") + __data()->name + ". Requested: " + Helper<T>::NAME() + ".";
     ...
     */
    template<typename T, typename std::enable_if<!std::is_base_of<IBasePointer, T>::value>::type* = nullptr>
    operator const T&() const
    {
     ...
     */
    template<template<typename...> typename T1, typename T, typename... TS, typename check = std::enable_if<std::is_base_of<IBasePointer, T1<T, TS...>>::value>::type>
    operator T1<T>()
    {
        if (is<T>())
        {
    ...
    bool to(T& v) const
    {
        ...
        else
        {
            ...
            if (TypeConverter::TryConvert(__data()->dataType, Helper<T>::ID(), __ptr(), &val))
            {
                v = val;
                return true;
            }
...
static auto __make_tuple_helper__(VALS&... vals)
{
...
#define DECLARE_DATA_TYPE(T)                                 \
    template<>                                               \
    struct data_core::Helper<T>                              \
    {                                                        \
        static constexpr size_t      ID() { return GID(T); } \
        static constexpr const char* NAME() { return #T; }   \
    };                                                       \
    template<>                                               \
    struct data_core::Helper<const T>                        \
    {                                                        \
        static constexpr size_t      ID() { return GID(T); } \
        static constexpr const char* NAME() { return #T; }   \
    };

#define DECLARE_DATA_TEMPLATE_TYPE(T, tf, ts)                           \
    template<tf>                                                        \
    struct data_core::Helper<T<ts>>                                     \
    {                                                                   \
        static constexpr size_t      ID() { return GID(T) ^ TGID(ts); } \
        static constexpr const char* NAME() { return #T; }              \
    };                                                                  \
    template<tf>                                                        \
    struct data_core::Helper<const T<ts>>                               \
    {                                                                   \
        static constexpr size_t      ID() { return GID(T) ^ TGID(ts); } \
        static constexpr const char* NAME() { return #T; }              \
    };

#define DECLARE_DATA_AUTO_TEMPLATE_TYPE(T)                                 \
    template<typename... Ts>                                               \
    struct data_core::Helper<T<Ts...>>                                     \
    {                                                                      \
        static constexpr size_t      ID() { return GID(T) ^ TGID(Ts...); } \
...
#define DECLARE_DATA_SERIALIZABLE(T)                                                                                          \
    template<>                                                                                                                \
    struct data_core::SerHelper<T>                                                                                            \
    {                                                                                                                         \
        static constexpr bool val = true;                                                                                     \
    };                                                                                                                        \
    template<>                                                                                                                \
    struct data_core::ser::imp_t<T>                                                                                           \
    {                                                                                                                         \
        static size_t Serialize(data_t& ret, const void* var) { return data_core::ser::SerializeImp<T>(ret, (const T*)var); } \
        static size_t Deserialize(const data_t& var, void* ret) { return data_core::ser::DeserializeImp<T>(var, (T*)ret); }   \
    };
// WARNING use this in global scope (without namespace)
#define DECLARE_DATA_SERIALIZABLE_TEMPLATE(T, tf, ts)            \
    template<tf>                                                 \
    struct data_core::SerHelper<T<ts>>                           \
    {                                                            \
        static constexpr bool val = true;                        \
    };                                                           \
    template<tf>                                                 \
    struct data_core::ser::imp_t<T<ts>>                          \
    {                                                            \
...
#define DECLARE_DATA_SERIALIZER_FUNC(T) \
    template<>                          \
    size_t data_core::ser::SerializeImp<T>(data_t & serealized, const T* void_deserealized)
// WARNING use this in global scope (without namespace)
#define DECLARE_DATA_DESERIALIZER_FUNC(T) \
    template<>                            \
    size_t data_core::ser::DeserializeImp<T>(const data_t& serealized, T* void_deserealized)

// WARNING use this in global scope (without namespace)
#define DECLARE_DATA_SERIALIZER_TEMPLATE_FUNC(T, tf, ts)    \
    DECLARE_DATA_SERIALIZABLE_TEMPLATE(T, __I(tf), __I(ts)) \
    template<tf>                                            \
    size_t data_core::ser::imp_t<T<ts>>::Serialize(data_t& serealized, const void* void_deserealized)
// WARNING use this in global scope (without namespace)
#define DECLARE_DATA_DESERIALIZER_TEMPLATE_FUNC(T, tf, ts) \
    template<tf>                                           \
    size_t data_core::ser::imp_t<T<ts>>::Deserialize(const data_t& serealized, void* void_deserealized)

#define DECLARE_DATA_SERIALIZER_DEFAULT_HEADER(T)        \
    const T* deserealized = (const T*)void_deserealized; \
    if (deserealized == 0)                               \
        throw std::invalid_argument("empty pointer");    \
    size_t offset = 0;                                   \
    Write(offset, serealized, data_core::Helper<T>::ID())

#define DECLARE_DATA_DESERIALIZER_DEFAULT_HEADER(T)                     \
    T* deserealized = (T*)void_deserealized;                            \
    if (deserealized == 0)                                              \
        throw std::invalid_argument("empty pointer");                   \
    size_t offset = 0;                                                  \
    if (Read<size_t>(offset, serealized) != data_core::Helper<T>::ID()) \
    throw std::invalid_argument("uncorrect uid")

// WARNING use this in global scope (without namespace)
#define DECLARE_DATA_SERIALIZER_DEFAULT(T)         \
    DECLARE_DATA_SERIALIZER_FUNC(T)                \
    {                                              \
...
// WARNING use this in global scope (without namespace)
#define DECLARE_DATA_DESERIALIZER_DEFAULT(T)         \
    DECLARE_DATA_DESERIALIZER_FUNC(T)                \
    {                                                \
...
// WARNING use this in global scope (without namespace)
#define DECLARE_DATA_SERIALIZER_DESERIALIZER_DEFAULT(T) \
    DECLARE_DATA_SERIALIZER_DEFAULT(T);                 \
    DECLARE_DATA_DESERIALIZER_DEFAULT(T);               \
    DECLARE_DATA_SERIALIZABLE(T);

// WARNING use this in global scope (without namespace)
#define DECLARE_DATA_SERIALIZER_TEMPLATE_DEFAULT(T, tf, ts) \
    DECLARE_DATA_SERIALIZER_FUNC(T, tf, ts)                 \
    {                                                       \
...
// WARNING use this in global scope (without namespace)
#define DECLARE_DATA_DESERIALIZER_TEMPLATE_DEFAULT(T, tf, ts) \
    DECLARE_DATA_DESERIALIZER_TEMPLATE_FUNC(T, tf, ts)        \
    {                                                         \
...
// WARNING use this in global scope (without namespace)
#define DECLARE_DATA_SERIALIZER_DESERIALIZER_TEMPLATE_DEFAULT(T, tf, ts) \
    DECLARE_DATA_SERIALIZER_TEMPLATE_DEFAULT(T, tf, ts);                 \
    DECLARE_DATA_DESERIALIZER_TEMPLATE_DEFAULT(T, tf, ts);

// WARNING use this in global scope (without namespace)
#define DECLARE_DATA_TYPE_AND_DEFAULT_SERIALIZER(T) \
    DECLARE_DATA_TYPE(T);                           \
    DECLARE_DATA_SERIALIZER_DESERIALIZER_DEFAULT(T);
...
#define REGISTER_CONVERSION_DEFAULT(From, To)                                                                                                                                                       \
    template<>                                                                                                                                                                                      \
    struct data_core::TypeConverterHelper<From, To>                                                                                                                                                 \
    {                                                                                                                                                                                               \
    ...
    namespace helpers                                                                                                                                                                               \
    {                                                                                                                                                                                               \
    static const bool REGISTER_CONVERSION_CONCAT(REGISTER_CONVERSION_CONCAT(_REGISTER_CONVERSION_STATIC_OBJECT_, __COUNTER__), __LINE__) = data_core::TypeConverter::RegisterConverter<From, To>(); \
    }                                                                                                                                                                                               \
    }

#define REGISTER_CONVERSION_FUNCTION(From, To, ...)                                                                                                                                                                                                        \
    template<>                                                                                                                                                                                                                                             \
    struct data_core::TypeConverterHelper<From, To>                                                                                                                                                                                                        \
    {                                                                                                                                                                                                                                                      \
    ...
    namespace helpers                                                                                                                                                                                                                                      \
    {                                                                                                                                                                                                                                                      \
    static const bool REGISTER_CONVERSION_CONCAT(REGISTER_CONVERSION_CONCAT(_REGISTER_CONVERSION_STATIC_OBJECT_, __COUNTER__), __LINE__) = data_core::TypeConverter::RegisterConverter<From, To>(&data_core::TypeConverterHelper<From, To>::ConvFunction); \
    }                                                                                                                                                                                                                                                      \
    }
...
#define __HELPER__(X)                              \
    REGISTER_CONVERSION_FUNCTION(X, std::string, { \
        try                                        \
        {                                          \
...
REGISTER_CONVERSION_FUNCTION(std::string, const char*, {
    try
    {
        to = from.c_str();
        return true;
...
REGISTER_CONVERSION_FUNCTION(std::string, long long, {
    try
    {
...
REGISTER_CONVERSION_FUNCTION(std::string, long, {
    try
    {
        to = std::stol(from.c_str());
        return true;
    } catch (const std::exception& e) {}
    return false;
});
REGISTER_CONVERSION_FUNCTION(std::string, int, {
    try
    {
        to = std::stoi(from.c_str());
        return true;
    } catch (const std::exception& e) {}
    return false;
});
REGISTER_CONVERSION_FUNCTION(std::string, short, {
    try
    {
        to = std::stoll(from.c_str());
        return true;
...
REGISTER_CONVERSION_FUNCTION(std::string, char, {
    try
    {
        to = (char)std::stoll(from.c_str());
        return true;
    } catch (const std::exception& e) {}
    return false;
});
REGISTER_CONVERSION_FUNCTION(std::string, unsigned long long, {
    try
    {
...
REGISTER_CONVERSION_FUNCTION(std::string, unsigned long, {
    try
    {
        to = std::stoul(from.c_str());
        return true;
    } catch (const std::exception& e) {}
    return false;
});
REGISTER_CONVERSION_FUNCTION(std::string, unsigned int, {
    try
    {
        to = std::stoul(from.c_str());
        return true;
...
REGISTER_CONVERSION_FUNCTION(std::string, unsigned short, {
    try
    {
        to = std::stoull(from.c_str());
        return true;
...
REGISTER_CONVERSION_FUNCTION(std::string, unsigned char, {
    try
    {
        to = (char)std::stoull(from.c_str());
        return true;
    } catch (const std::exception& e) {}
    return false;
});
REGISTER_CONVERSION_FUNCTION(std::string, float, {
    try
    {
...
REGISTER_CONVERSION_FUNCTION(std::string, double, {
    try
    {
...
DECLARE_DATA_SERIALIZER_TEMPLATE_FUNC(std::vector, __I(typename T), __I(T))
{
    ...
    Write(offset, serealized, (int)deserealized->size());
    for (int i = 0; i < deserealized->size(); ++i) SerWrite(offset, serealized, deserealized->data() + i);
    return offset;
}
DECLARE_DATA_DESERIALIZER_TEMPLATE_FUNC(std::vector, __I(typename T), __I(T))
{
    ...
    deserealized->resize(Read<int>(offset, serealized));
    for (int i = 0; i < deserealized->size(); ++i) DserRead(offset, serealized, deserealized->data() + i);
    return offset;
}
...
DECLARE_DATA_SERIALIZER_TEMPLATE_FUNC(std::list, __I(typename T), __I(T))
{
    ...
    Write(offset, serealized, (int)deserealized->size());
    for (auto& el : deserealized) SerWrite(offset, serealized, &el);
    return offset;
}
DECLARE_DATA_DESERIALIZER_TEMPLATE_FUNC(std::list, __I(typename T), __I(T))
{
    ...
    deserealized->resize(Read<int>(offset, serealized));
    for (auto& el : deserealized) DserRead(offset, serealized, &el);
    return offset;
}
...
DECLARE_DATA_SERIALIZER_TEMPLATE_FUNC(std::tuple, __I(typename... ARGS), __I(ARGS...))
{
    ...
    std::apply([&](auto&&... args) { (SerWrite(offset, serealized, &args), ...); }, *deserealized);
    return offset;
}
...
DECLARE_DATA_DESERIALIZER_TEMPLATE_FUNC(std::tuple, __I(typename... ARGS), __I(ARGS...))
{
    ...
    if (Read<int>(offset, serealized) != (int)sizeof...(ARGS))
        throw std::invalid_argument("uncorrect size");
    std::apply([&](auto&&... args) { (DserRead(offset, serealized, &args), ...); }, *deserealized);
    return offset;
}

// struct

#define DECLARE_DATA_STRUCT_AND_SERIALIZE(name, ...)                                             \
    DECLARE_DATA_TYPE(name);                                                                     \
    DECLARE_DATA_SERIALIZER_FUNC(name)                                                           \
    {                                                                                            \
        DECLARE_DATA_SERIALIZER_DEFAULT_HEADER(name);                                            \
        struct __HELPER__ : name                                                                 \
        {                                                                                        \
            using tuple_type = decltype(std::tuple(__VA_ARGS__));                                \
            auto make_tuple() const { return std::tuple(__VA_ARGS__); }                          \
        };                                                                                       \
    ...
    DECLARE_DATA_DESERIALIZER_FUNC(name)                                                         \
    {                                                                                            \
        DECLARE_DATA_DESERIALIZER_DEFAULT_HEADER(name);                                          \
        struct __HELPER__ : name                                                                 \
        {                                                                                        \
            using tuple_type = decltype(std::tuple(__VA_ARGS__));                                \
            auto make_tuple() { return ser::__make_tuple_helper__(__VA_ARGS__); }                \
        };                                                                                       \
        if (Read<size_t>(offset, serealized) != data_core::Helper<__HELPER__::tuple_type>::ID()) \
            throw std::invalid_argument("uncorrect tuple uid");                                  \
        if (Read<int>(offset, serealized) != std::tuple_size_v<__HELPER__::tuple_type>)          \
            throw std::invalid_argument("uncorrect tuple size");                                 \
